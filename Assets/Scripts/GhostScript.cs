...
public class GhostScript : MonoBehaviour
{
    ...
    private struct StartPosition
    {
        ...
        public StartPosition(Vector3 pos, Quaternion rot)
        {
            position = pos;
            rotation = rot;
        }
    ...
    void Start()
    {
        startPos = new StartPosition(this.transform.position, this.transform.rotation);
        Anim = this.GetComponent<Animator>();
        Ctrl = this.GetComponent<CharacterController>();
        HP_text = GameObject.Find("Canvas/HP").GetComponent<Text>();
        HP_text.text = "HP " + HP.ToString();

        GameObject textObjC = GameObject.FindGameObjectWithTag("TextCoins");
        if (textObjC != null)
        {
            coinCountText = textObjC.GetComponent<Text>();
        } else {Debug.LogError("Не найден объект с тегом 'TextCoins'!");}
        GameObject textObjD = GameObject.FindGameObjectWithTag("TextDeaths");
        if (textObjD != null)
        {
            deathCountText = textObjD.GetComponent<Text>();
        } else {Debug.LogError("Не найден объект с тегом 'TextDeaths'!");}

        LevelGenerator levelGen = FindAnyObjectByType<LevelGenerator>();
        if (levelGen != null)
        {
            allCoins = levelGen.AllCoins;
        }
        GameObject vict = GameObject.FindGameObjectWithTag("Finish");
        if (vict != null)
        {
            victoryPanel = vict;
        }
    ...
    private void Damage ()
    {
        // Damaged by outside field. или лавой
        if(Input.GetKeyUp(KeyCode.S) || lavaDamage)
        {
    ...
    {
        if(Input.GetKeyDown(KeyCode.Space) || respawnFlag)
        {
            lavaDamage = false;
            respawnFlag = false;
            // player HP
    ...
    public void AddCoin()
    {
        ...
        {
            coinCountText.text = "Монет: " + coinCount;
        }
        // Проверяем, собраны ли все монеты
        if (coinCount >= allCoins)
        {
    ...
    private void AddDeath()
    {
        ...
        if (deathCountText != null)
            {
                deathCountText.text = "Смерти: " + deathCount;
            }
    }
}
}